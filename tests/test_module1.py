...
def test_task2_module1():
    assert 'expenses:Expense:Expenses' in get_assignments(
        FrequentExpenses), 'Do you have a `Expense.Expenses()` constructor call?'
    assert 'expenses:read_expenses:data/spending_data.csv' in get_calls(
        FrequentExpenses), 'Are you calling the `read_expenses()` method with `".data/spending_data.csv"`?'

# spending_categories = []
@pytest.mark.test_task3_module1
def test_task3_module1():
    assert 'spending_categories' in get_assignments(
        FrequentExpenses), 'Are you initializing `spending_categories` to an empty list?'
...
def test_task4_module1():
    ...
    assert 'spending_counter:collections:Counter:spending_categories' in get_assignments(
        FrequentExpenses), 'Did you call `collections.Counter()` with argument `spending_categories` and assign to `spending_counter`?'

# top5 = spending_counter.most_common(5)
@pytest.mark.test_task5_module1
def test_task5_module1():
    assert 'top5:spending_counter:most_common:5' in get_assignments(
        FrequentExpenses), 'Did you call `spending_counter.most_common(5)` and assign to a variable `top5`?'

# categories, count = zip(*top5)
@pytest.mark.test_task6_module1
def test_task6_module1():
    assert 'categories:count:zip:top5' in get_assignments(
        FrequentExpenses), 'Did you call `zip(*top5)` and assign to variables `categories, count`?'
...
def test_task7_module1():
    ...
    assert 'fig:ax:plt:subplots' in get_assignments(
        FrequentExpenses), 'Did you call `plt.subplots()` and assign to variables `fig, ax`?'

# fig,ax=plt.subplots()
@pytest.mark.test_task8_module1
def test_task8_module1():
    # ax.bar(categories, count)
    assert 'ax:bar:categories:count' in get_calls(
        FrequentExpenses), 'Did you call `ax.bar()` with arguments `categories, count`?'
    # ax.set_title('# of Purchases by Category')
    assert 'ax:set_title:# of Purchases by Category' in get_calls(
        FrequentExpenses), 'Did you call `ax.set_title()` with argument `"# of Purchases by Category"`?'
